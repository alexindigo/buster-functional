...
function busterFunctionalMixin(testCase, options)
{
  ...
  // Cooldown period, default pausing time for
  // recurrent waiting procedures
  ...
  // like typing (keydown, keyup) and touching (touchstart, touchend)
  ...
  // transitionEnd multi-name, allow to adjust
  ...
  testCase.load = function busterFunctionalMixin_load(uriPath, callback)
  {
    ...
    // should be gone when buster-proxy allows un-prefixed requests
  ...
  testCase.touch = function busterFunctionalMixin_touch(target, callback)
  {
    ...
    setTimeout(function()
    {
      ...
      this._triggerEvents(target,
        [
          'touchstart',
          ['touchend', 'mouseover', 'mousemove', 'mousedown', 'focus', 'focusin', 'DOMFocusIn', 'mouseup', 'click']
        ], target, callback);
    ...
    setTimeout(function()
    {
      this._triggerEvents(target,
        [
          ...
          'mousemove',
          ['mousedown', 'focus', 'focusin', 'DOMFocusIn'],
          ['mouseup', 'click']
        ], target, callback);
    }.bind(this), this._delay);
  ...
  testCase.select = function busterFunctionalMixin_select(target, option, callback)
  {
    ...
    setTimeout(function()
    {
      this._triggerEvents(target,
        [
          ['focus', 'focusin', 'DOMFocusIn'],
          [selectOption, 'input', 'change'],
          ['blur', 'focusout', 'DOMFocusOut']
        ], callback);
    }.bind(this), this._delay);
  ...
  testCase.checkbox = function busterFunctionalMixin_checkbox(target, callback)
  {
    ...
    setTimeout(function()
    {
      this._triggerEvents(target,
        [
          'touchstart',
          ['touchend', 'mouseover', 'mousedown', 'focus', 'focusin', 'DOMFocusIn', 'mouseup', selectCheckbox, 'change', 'click'],
          ['mouseout', 'blur', 'focusout', 'DOMFocusOut']
        ], target, callback);
    }.bind(this), this._delay);
  ...
  testCase.multipleCheckbox = function busterFunctionalMixin_multipleCheckbox(target, callback)
  {
    ...
    function selectMultipleCheckboxes() {
      ...
      {
        this._triggerEvents(el,
          [
            'touchstart',
            ['touchend', 'mouseover', 'mousedown', 'focus', 'focusin', 'DOMFocusIn', 'mouseup', selectCheckbox, 'change', 'click'],
            ['mouseout', 'blur', 'focusout', 'DOMFocusOut']
          ], el, selectMultipleCheckboxes.bind(this));
      }
      else
      {
        this._delayedCallback(callback);
  ...
  {
    // check if target element exists
    if (typeof this.window == 'object' && (variable in this.window))
    {
      // and it's async
      this._delayedCallback(callback);
      return this;
  ...
  testCase._once = function functional_mixin__once(fn)
  {
    var oneTimer = function()
    {
      if (!oneTimer.called)
      {
        ...
        oneTimer.result = fn.apply(this, arguments);
      }

      return oneTimer.result;
    };
  ...
  // as second dimension array
  // Extra parameters will passed to the specific events
  ...
  testCase._createEvent = function busterFunctionalMixin__createEvent(type)
  {
    var handler;
